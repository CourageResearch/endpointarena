...
const TABLE_STATEMENTS = [
  ...
  `CREATE INDEX IF NOT EXISTS market_run_logs_run_created_idx ON market_run_logs(run_id, created_at)`,
  `CREATE INDEX IF NOT EXISTS market_run_logs_created_at_idx ON market_run_logs(created_at)`,
  ...
  `CREATE TABLE IF NOT EXISTS model_decision_snapshots (
    id text PRIMARY KEY,
    market_id text NOT NULL REFERENCES prediction_markets(id) ON DELETE CASCADE,
    fda_event_id text NOT NULL REFERENCES fda_calendar_events(id) ON DELETE CASCADE,
    model_id text NOT NULL,
    ...
    proposed_amount_usd real NOT NULL DEFAULT 0,
    ...
    inference_geo text,
    linked_market_action_id text REFERENCES market_actions(id) ON DELETE SET NULL,
    created_at timestamp DEFAULT NOW(),
    CONSTRAINT model_decision_snapshots_run_source_check CHECK (run_source IN ('manual', 'cycle')),
    CONSTRAINT model_decision_snapshots_binary_call_check CHECK (binary_call IN ('approved', 'rejected')),
    CONSTRAINT model_decision_snapshots_confidence_check CHECK (confidence >= 50 AND confidence <= 100),
    CONSTRAINT model_decision_snapshots_approval_probability_check CHECK (approval_probability >= 0 AND approval_probability <= 1),
    CONSTRAINT model_decision_snapshots_proposed_action_check CHECK (proposed_action_type IN ('BUY_YES', 'BUY_NO', 'SELL_YES', 'SELL_NO', 'HOLD')),
    CONSTRAINT model_decision_snapshots_proposed_amount_check CHECK (proposed_amount_usd >= 0),
    CONSTRAINT model_decision_snapshots_market_price_yes_check CHECK (market_price_yes IS NULL OR (market_price_yes >= 0 AND market_price_yes <= 1)),
    CONSTRAINT model_decision_snapshots_market_price_no_check CHECK (market_price_no IS NULL OR (market_price_no >= 0 AND market_price_no <= 1)),
    CONSTRAINT model_decision_snapshots_cash_available_check CHECK (cash_available IS NULL OR cash_available >= 0),
    CONSTRAINT model_decision_snapshots_yes_shares_held_check CHECK (yes_shares_held IS NULL OR yes_shares_held >= 0),
    CONSTRAINT model_decision_snapshots_no_shares_held_check CHECK (no_shares_held IS NULL OR no_shares_held >= 0),
    CONSTRAINT model_decision_snapshots_max_buy_usd_check CHECK (max_buy_usd IS NULL OR max_buy_usd >= 0),
    CONSTRAINT model_decision_snapshots_max_sell_yes_usd_check CHECK (max_sell_yes_usd IS NULL OR max_sell_yes_usd >= 0),
    CONSTRAINT model_decision_snapshots_max_sell_no_usd_check CHECK (max_sell_no_usd IS NULL OR max_sell_no_usd >= 0),
    CONSTRAINT model_decision_snapshots_cost_source_check CHECK (cost_source IS NULL OR cost_source IN ('provider', 'estimated'))
  )`,
  `CREATE INDEX IF NOT EXISTS model_decision_snapshots_event_model_created_idx ON model_decision_snapshots(fda_event_id, model_id, created_at)`,
  `CREATE INDEX IF NOT EXISTS model_decision_snapshots_market_model_created_idx ON model_decision_snapshots(market_id, model_id, created_at)`,
  `CREATE INDEX IF NOT EXISTS model_decision_snapshots_run_source_idx ON model_decision_snapshots(run_source)`,
  ...
  `ALTER TABLE market_runtime_configs ALTER COLUMN created_at SET DEFAULT NOW()`,
  `ALTER TABLE market_runtime_configs ALTER COLUMN updated_at SET DEFAULT NOW()`,
  `INSERT INTO market_runtime_configs (id, created_at, updated_at) VALUES ('default', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
