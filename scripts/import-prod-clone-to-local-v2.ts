...
async function main() {
  ...
  try {
    const [
      ...
      sourceSql`select * from fda_calendar_events order by pdufa_date asc, id`,
    ...
    const importedEvents = sourceEvents.map((row: any) => ({
      ...
      pdufaDate: normalizeUtcDate(row.pdufa_date),
      eventDescription: row.event_description,
      outcome: row.outcome,
      outcomeDate: row.outcome_date,
      dateKind: normalizeString(row.date_kind) === 'synthetic' ? 'synthetic' : 'public',
