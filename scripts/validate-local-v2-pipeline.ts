...
import { normalizeRunDate, reopenMarketForEvent, resolveMarketForEvent, runBuyAction } from '../lib/markets/engine'
...
function buildMockDecision(modelId: ModelId, input: ModelDecisionInput): ModelDecisionGeneration {
  ...
  const binaryCall = boundedProbability >= 0.5 ? 'approved' as const : 'rejected' as const
  const maxBuyUsd = Math.max(0, Math.min(input.portfolio.maxBuyUsd, 120 + (modelIndex * 5)))
  const actionType = maxBuyUsd > 0
    ? (binaryCall === 'approved' ? 'BUY_YES' : 'BUY_NO')
    : 'HOLD'
...
async function main() {
  ...
  if (!market) {
    throw new Error('No open market found in local v2 DB')
  }

  const event = await db.query.fdaCalendarEvents.findFirst({
    where: eq(fdaCalendarEvents.id, market.fdaEventId),
  })
  if (!event) {
  ...
  await resolveMarketForEvent(event.id, 'Approved')
  const resolved = await db.query.predictionMarkets.findFirst({
    where: eq(predictionMarkets.id, market.id),
  })
  if (!resolved || resolved.status !== 'RESOLVED' || resolved.resolvedOutcome !== 'Approved') {
    throw new Error('Market did not resolve correctly')
  }

  await reopenMarketForEvent(event.id)
  ...
  const email = `local-v2-${Date.now()}@example.com`
  ...
  const firstHumanTrade = await runBuyAction({
    ...
    side: 'BUY_YES',
    requestedUsd: 100,
    explanation: 'Local v2 validation trade',
    actionSource: 'human',
  })
  const secondHumanTrade = await runBuyAction({
    ...
    side: 'BUY_YES',
    requestedUsd: 50,
    explanation: 'Local v2 repeat validation trade',
    actionSource: 'human',
  })
