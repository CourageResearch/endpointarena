...
async function assertBaseSchemaPresent() {
  ...
  if (!exists) {
    throw new Error('Base schema is missing. Run db:push-local before bootstrap.')
  }
}
...
async function main() {
  ...
  const [event] = await db.insert(fdaCalendarEvents).values({
    ...
    decisionDate: parseUtcDate(seed.publicActionDate || '2026-06-20'),
    decisionDateKind: seed.publicActionDate ? 'hard' : 'soft',
